...
export default function ChatInterface() {
  ...
  const renderMessage = (message: Message) => {
    ...
    return (
      <div key={message.id} className={cn("flex flex-col items-start")}>
        <div
          className={cn(
          ...
          >
            ...
            <div className="py-4 flex-none">
              ...
              <Button
                ...
                size="icon"
                className={cn(
                  "rounded-full h-6 w-6 border-0 flex-shrink-0 transition-all duration-200",
                  hasTyped ? "bg-black scale-110" : "bg-transparent",
                )}
                disabled={!inputValue.trim() || isStreaming}
              >
                <ArrowUp className={cn("h-4 w-4 transition-colors", hasTyped ? "text-gray-200" : "text-gray-400")} />
